// pocao.c - VERSÃO PORTÁVEL

#include <stdio.h>
#include <stdlib.h>
// REMOVIDO: #include <windows.h> - substituído por portabilidade.h
#include "pocao.h"
#include "playgame.h" 
#include "cores.h" 
#include "desenhos.h" 
...
// Incluindo o novo cabeçalho de portabilidade
...
void aplicarPorcao(struct usuario *heroi, int idPorcao) {
    ...
    // LINHA ADAPTADA: Substituindo Sleep(1000)
    PausaPortavel(1000); 
}
...
void menuPorcoes(struct usuario *heroi) {
    ...
    // LINHA ADAPTADA: Substituindo system("CLS")
    LimparTelaPortavel();
    ...
    if (scanf("%d", &escolha_porcao) != 1) {
        ...
        // LINHA ADAPTADA: Substituindo Sleep(500)
        PausaPortavel(500);
        return;
    }
    limparBuffer();
    ...
    if (escolha_porcao >= 1 && escolha_porcao <= NUM_PORCOES) {
        ...
        if (heroi->usosPocao >= LIMITE_USO_PORCAO) {
            ...
            mudarCor(7);
            // LINHA ADAPTADA: Substituindo Sleep(1500)
            PausaPortavel(1500);
            return; 
        }
